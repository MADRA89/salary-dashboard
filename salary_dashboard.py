...
def mock_parse_cv_and_jd(): return { "educationScore": random.choice([10, 7, 4, 2]), "experienceScore": random.choice([10, 7, 5, 3]) }

def mock_parse_interview_sheet(): return { "performanceScore": random.choice([10, 7, 5, 2]) }

def get_step_interval(score): if score >= 25: return list(range(12, 16)), "Top Range" elif score >= 20: return list(range(9, 12)), "Mid-Upper Range" elif score >= 15: return list(range(6, 9)), "Mid Range" elif score >= 10: return list(range(3, 6)), "Lower-Mid Range" else: return list(range(1, 3)), "Bottom Range"

def convert_df_to_excel(df): output = BytesIO() with ExcelWriter(output, engine="openpyxl") as writer: df.to_excel(writer, sheet_name="EquityAnalysis", index=False) return output.getvalue()

def load_filtered_equity_data(uploaded_file, position_title_input): df = pd.read_excel(uploaded_file) df.columns = [col.strip().lower() for col in df.columns] required_cols = {'id', 'position title', 'hire date', 'comp rate'} if not required_cols.issubset(set(df.columns)): return None, "❌ Excel must include columns: ID, Position Title, Hire Date, Comp Rate." df_filtered = df[[c for c in df.columns if c in required_cols]] df_filtered.columns = ['id', 'positionTitle', 'hireDate', 'compRate'] df_filtered = df_filtered[df_filtered['positionTitle'].str.lower() == position_title_input.lower()] return df_filtered, None

def generate_word_report(name, title, grade, education_score, experience_score, performance_score, total_score, interval_options, placement, selected_step, recommended_salary, final_salary, budget_threshold, hr_comments, df_peers): doc = Document() doc.add_heading('Final Salary Evaluation Report', 0)
...
st.set_page_config(page_title="Salary Evaluation Dashboard", layout="wide") st.markdown("<h1 style='color:#003366;'>📊 Salary Evaluation Dashboard</h1>", unsafe_allow_html=True)

tab1, tab2 = st.tabs(["📘 Evaluation & Scoring Matrices", "📋 Candidate Analysis"])

with tab1: col1, col2 = st.columns(2) with col1: st.subheader("🎓 Candidate Evaluation Matrix") st.markdown("""
...
with tab2: st.subheader("Step 1: Candidate & Position Details") colA, colB = st.columns([1, 2])
...
if uploaded_cv and uploaded_jd and not st.session_state.ai_scores_initialized:
    with st.spinner("🔍 Evaluating CV & JD..."):
        time.sleep(1)
        parsed = mock_parse_cv_and_jd()
        st.session_state.educationScore = parsed["educationScore"]
        st.session_state.experienceScore = parsed["experienceScore"]
        st.session_state.ai_scores_initialized = True

if uploaded_interview and "performanceScore" not in st.session_state:
    with st.spinner("🧠 Evaluating Interview..."):
        time.sleep(1)
        st.session_state.performanceScore = mock_parse_interview_sheet()["performanceScore"]

if "educationScore" not in st.session_state:
    st.session_state.educationScore = 0
if "experienceScore" not in st.session_state:
    st.session_state.experienceScore = 0
if "performanceScore" not in st.session_state:
    st.session_state.performanceScore = 0

education_score = st.slider("🎓 Education Score (Editable)", 0, 10, st.session_state.educationScore, key="education_slider")
experience_score = st.slider("💼 Experience Score (Editable)", 0, 10, st.session_state.experienceScore, key="experience_slider")
performance_score = st.slider("🚀 Performance Score (Editable)", 0, 10, st.session_state.performanceScore, key="performance_slider")
...
st.subheader("Step 4: Salary Recommendation") budget_threshold = st.number_input("💰 Budget Threshold (AED)", step=500) recommended_salary = st.number_input("🤖 AI-Recommended Salary (AED)", step=500) final_salary = st.number_input("✅ Final Recommended Salary (AED)", step=500)

st.subheader("Step 5: Internal Equity Analysis") if uploaded_equity and title and final_salary == 0: st.info("ℹ️ Please enter the Final Recommended Salary (AED) above to perform the equity analysis.") df_peers = None if uploaded_equity and title and final_salary > 0: df_peers, error = load_filtered_equity_data(uploaded_equity, title) if error: st.error(error) elif df_peers.empty: st.warning(f"No matching peers found for position title: '{title}'") else: df_peers = pd.concat([ df_peers, pd.DataFrame([{ "id": "Candidate", "positionTitle": title, "hireDate": "N/A", "compRate": final_salary }]) ], ignore_index=True)
...
st.subheader("Step 6: Budget Status") if final_salary and budget_threshold: if final_salary <= budget_threshold: st.success(f"✅ Within Budget (AED {final_salary:,.0f})") else: st.error(f"❌ Out of Budget (AED {final_salary:,.0f})")

st.subheader("Step 7: Final HR Comments & Export") hr_comments = st.text_area("📝 HR Final Comments")

if st.button("📤 Generate Final Summary"): summary = f""" 📌 Final Recommendation Summary
